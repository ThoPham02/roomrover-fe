...
const Breadcrumbs = ({ backRoute, backName, displayName }) => {
  return (
    <div className="d-flex align-items-center py-2">
      <FaRegFlag
        className={`${displayName ? "text-orange" : "text-orange"} text-2xl`}
      />
        ...
        <ol className="breadcrumb m-0 p-0 d-flex list-unstyled">
          {backName && (
            <li className="breadcrumb-item">
              {displayName ? (
                <Link to={backRoute} className="text-orange font-bold">
                  {backName}
                </Link>
              ) : (
                <span className="text-orange font-bold">{backName}</span>
              )}
            </li>
          )}
          {displayName && (
